 ...
 *
 * Set these via environment variables (VITE_FAL_AI_PROXY_URL, etc.)
 * or they will use the IMG.LY demo proxies when available.
 */
let FAL_AI_PROXY_URL = import.meta.env.VITE_FAL_AI_PROXY_URL || '';
let ANTHROPIC_PROXY_URL = import.meta.env.VITE_ANTHROPIC_PROXY_URL || '';
let OPEN_AI_PROXY_URL = import.meta.env.VITE_OPENAI_PROXY_URL || '';
let ELEVENLABS_PROXY_URL = import.meta.env.VITE_ELEVENLABS_PROXY_URL || '';
  ...
  text2image: {
    ...
    providers: [
      ...
      },
      {
        ...
        label: 'Google',
        selected: false,
        provider: () => FalAiImage.GeminiFlash25({ proxyUrl: FAL_AI_PROXY_URL })
      }
    ]
  },
  ...
  image2image: {
    ...
    providers: [
      ...
      },
      {
        ...
        label: 'Alibaba',
        selected: false,
        provider: () => FalAiImage.QwenImageEdit({ proxyUrl: FAL_AI_PROXY_URL })
      },
      {
        ...
        selected: false,
        provider: () =>
          FalAiImage.Gemini25FlashImageEdit({ proxyUrl: FAL_AI_PROXY_URL })
      }
    ]
  },
