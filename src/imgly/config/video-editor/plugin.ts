...
export class VideoEditorConfig implements EditorPlugin {
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // This must be called AFTER the editor is initialized.
      // - videoDecode: 'block' - Block editor if browser cannot decode video
      // - videoEncode: 'warn' - Warn user if browser cannot encode video
